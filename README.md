...
The CRM uses separate Vercel projects for the Next.js app, the NestJS API, and the eve agent. The Next.js app runs on Vercel natively. The API uses the committed Node serverless entrypoint in `apps/api/api/index.ts` and the Vercel Build Output generated by `bun run --cwd apps/api build:vercel`; it does not run the long-lived local Bun server in `src/main.ts`.

Set `API_URL` and `APP_URL` to the real origins. The API and app projects must share `DATABASE_URL` and `BETTER_AUTH_SECRET` because the API mints session cookies and the app reads sessions directly. Use a pooled `DATABASE_URL` for runtime queries and `DIRECT_DATABASE_URL` for build-time migrations. Set `CRON_SECRET` for the Google sync cron and configure `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` together on the API project.

For a production API at `https://api.example.com`, the Google OAuth callback is:

```text
https://api.example.com/api/auth/callback/google
```

See [`docs/vercel.md`](./docs/vercel.md) for project roots, environment variables, migration safety, and preview/custom-domain requirements.

`apps/api/src/generated/server.ts` is committed and `build` must never regenerate it — the generator needs a newer GLIBC than most build images have. Regenerate locally and commit it with the router change that caused it.
